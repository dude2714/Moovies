...
getLinkCine = () => {
    interval = setInterval(getDataCine, 300);
}

getDataCine = () => {
    let links = jwplayer().getPlaylistItem();
...
getLinkSolar = () =>{
    interval = setInterval(getDataSolar, 300);
}
...
getDataSoap = () => {
    let urlVideo = document.getElementsByClassName('jw-video jw-reset')[0].src;
    if(urlVideo !== undefined && urlVideo.length > 0){
        clearInterval(interval);
        Android.returnLink(urlVideo);
    }else{
        count = count + 1;
