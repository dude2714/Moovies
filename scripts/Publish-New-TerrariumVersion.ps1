...
$sourceFull = [System.IO.Path]::GetFullPath((Resolve-Path -LiteralPath $NewApkPath))
$targetFull = [System.IO.Path]::GetFullPath($targetApk)

# Avoid self-copy failures when publishing from the already-canonical APK path.
...
Write-Host "Updated Terrarium TV release"
